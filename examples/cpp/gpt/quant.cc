...
int main(int argc, char* argv[])
{
    ...
    //     host_quantized_weight_buf.data(), host_scales_buf.data(), host_array.data(), shape,
    ...
    // for (int i = 0; i < 70; ++i) {
    //   printf("%d %d * %5f -> %5f vs %5f\n", i, host_quantized_weight_buf[i], host_scales_buf[i%shape[1]],
    //   host_array[i], host_scales_buf[i%shape[1]] * float(host_quantized_weight_buf[i]));
    // }
    ...
    // //     printf("low=%5f, high=%5f\n", low * host_scales_buf[jj*2 % shape[1]], high * host_scales_buf[(jj*2+1) %
    // shape[1] ]);
    // // }
