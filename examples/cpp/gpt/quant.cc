...
{
    std::string filename =
        "../models/openai-gpt-models/c-model/124m/1-gpu//model.layers.0.attention.query_key_value.weight.0.bin";

    std::vector<size_t> shape{12*64, 12*3*64};

    std::vector<float>   host_array = loadWeightFromBinHelper<float>(shape, filename);
